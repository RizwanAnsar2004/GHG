...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Store(id='zoom-level', data=3)  # Store the current zoom level
...
@app.callback(
    ...
    Input('emission-checkboxes', 'value'),
    Input('year-slider', 'value'),
    Input('zoom-in', 'n_clicks'),
    Input('zoom-out', 'n_clicks'),
    Input('zoom-level', 'data')
)
def update_heatmap(selected_years, selected_vars, selected_year, zoom_in, zoom_out, current_zoom):
    df_list = []
    ...
    # Update zoom level based on button clicks
    if zoom_in > 0:
        current_zoom += 1
    elif zoom_out > 0:
        current_zoom -= 1
    ...
    fig = px.density_mapbox(
        ...
        zoom=current_zoom,  # Use the current zoom level
        opacity=0.6,
        color_continuous_scale=px.colors.sequential.Plasma
    )

    return fig, current_zoom  # Return the updated zoom level
